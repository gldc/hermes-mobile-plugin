...
- **Tokens are hashed at rest.** `devices.json` stores SHA-256 hashes
  ...
  rotate on every use, and replaying an *older* rotated-out token
  (two+ rotations back) revokes the device (stolen-token containment).
  A replay of the *immediately-prior* token is forgiven and re-rotated —
